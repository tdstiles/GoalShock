...
from backend.bot.market_fetcher import (  # noqa: E402
    MarketFetcher,
    POLY_MSG_TYPE_PRICE_UPDATE,
    KALSHI_MSG_TYPE_SNAPSHOT,
)
from backend.models.schemas import MarketPrice, MarketUpdate  # noqa: E402
...
@pytest.mark.asyncio
async def test_process_polymarket_update_updates_cache(
    market_fetcher: MarketFetcher,
) -> None:
    """Update polymarket cache for valid updates.
...
@pytest.mark.asyncio
async def test_process_polymarket_update_notifies_callbacks(
    market_fetcher: MarketFetcher,
) -> None:
    """Notify callbacks for valid updates on polymarket.
...
@pytest.mark.asyncio
async def test_process_kalshi_update_normalizes_prices(
    market_fetcher: MarketFetcher,
) -> None:
    """Normalize kalshi prices for updates.
...
@pytest.mark.asyncio
async def test_process_kalshi_update_notifies_callbacks(
    market_fetcher: MarketFetcher,
) -> None:
    """Notify callbacks for valid updates on kalshi.
...
@pytest.mark.asyncio
async def test_apply_market_update_updates_cache(
    market_fetcher: MarketFetcher,
) -> None:
    """Update cache entries through the helper.
    ...
    await market_fetcher._apply_market_update(
        "mkt-5", yes_price=0.62, no_price=0.38
    )
...
@pytest.mark.asyncio
async def test_apply_market_update_notifies_callbacks(
    market_fetcher: MarketFetcher,
) -> None:
    """Notify callbacks through the helper.
...
@pytest.mark.asyncio
async def test_fetch_markets_from_configured_sources(
    market_fetcher: MarketFetcher, monkeypatch
) -> None:
    """Test fetching markets from configured sources.
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=[poly_market]),
    )
    monkeypatch.setattr(
        market_fetcher,
        "_fetch_kalshi_markets",
        AsyncMock(return_value=[kalshi_market]),
    )
...
@pytest.mark.asyncio
async def test_fetch_markets_caches_from_configured_sources(
    market_fetcher: MarketFetcher, monkeypatch
) -> None:
    """Test caching markets from configured sources.
    ...
    await market_fetcher.fetch_markets_for_fixture(
        fixture_id=1, home_team="Team A", away_team="Team B"
    )
...
@pytest.mark.asyncio
async def test_fetch_markets_handles_api_errors(
    market_fetcher: MarketFetcher, monkeypatch
) -> None:
