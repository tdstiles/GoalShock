...
def mock_dependencies():
    with patch('engine_unified.PolymarketClient') as mock_poly, \
         ...
         patch('engine_unified.WebSocketGoalListener') as mock_ws, \
         patch('engine_unified.HybridGoalListener') as mock_hybrid, \
        ...
        mock_alpha1.return_value.on_goal_event = AsyncMock(return_value=MagicMock(signal_id="sig1"))
...
async def test_engine_start_stop(mock_dependencies):
    ...
    config = EngineConfig(enable_websocket=False) # Disable WS to avoid needing that mock
    engine = UnifiedTradingEngine(config)
    ...
    with patch.object(engine, '_pre_match_odds_loop', new_callable=AsyncMock) as mock_loop1, \
         patch.object(engine, '_live_fixture_loop', new_callable=AsyncMock) as mock_loop2, \
         patch.object(engine, '_stats_reporter_loop', side_effect=run_until_stopped) as mock_loop3:
