...
class EngineConfig:
    ...
    def from_env(cls) -> "EngineConfig":
        ...
        return cls(
            mode=mode,
            enable_alpha_one=os.getenv(ENV_ENABLE_ALPHA_ONE, VAL_TRUE).lower()
            == VAL_TRUE,
            enable_alpha_two=os.getenv(ENV_ENABLE_ALPHA_TWO, VAL_TRUE).lower()
            == VAL_TRUE,
            enable_websocket=os.getenv(ENV_ENABLE_WEBSOCKET, VAL_TRUE).lower()
            == VAL_TRUE,
...
class UnifiedTradingEngine:
    ...
    def __init__(self, config: Optional[EngineConfig] = None):
        ...
        logger.info(
            f"Alpha One (Underdog): {'ENABLED' if self.config.enable_alpha_one else 'DISABLED'}"
        )
        logger.info(
            f"Alpha Two (Clipping): {'ENABLED' if self.config.enable_alpha_two else 'DISABLED'}"
        )
        logger.info(
            f"WebSocket: {'ENABLED' if self.config.enable_websocket else 'DISABLED'}"
        )
        logger.info(
            f"Polymarket: {'CONNECTED' if self.polymarket else 'NOT CONFIGURED'}"
        )
    ...
    async def _on_fixture_update(self, fixtures: List[LiveFixture]):
        ...
        async def process_fixture(fixture: LiveFixture) -> None:
            ...
            try:
                ...
                logger.error(
                    f"Error processing fixture {fixture.fixture_id}: {e}", exc_info=True
                )
    ...
    async def _get_fixture_market_prices(
        ...
        try:
            ...
            logger.warning(
                f"Price not found for token {token_id} (Fixture: {fixture.fixture_id})"
            )

        except Exception as e:
            logger.error(
                f"Error fetching market prices for {event_name}: {e}", exc_info=True
            )
        ...
        while self.running:
            try:
                ...
                if self.alpha_one:
                    ...
                    logger.info(
                        f"Alpha One - Trades: {stats.total_trades}, Win Rate: {stats.win_rate:.1%}, P&L: ${stats.total_pnl:.2f}"
                    )

                if self.alpha_two:
                    stats = self.alpha_two.get_stats()
                    logger.info(
                        f"Alpha Two - Trades: {stats.trades_executed}, Win Rate: {stats.win_rate:.1%}, P&L: ${stats.total_pnl:.2f}"
                    )
...
def parse_cli_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    Args:
        argv: Optional list of CLI tokens. If omitted, argparse reads sys.argv.
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Enable/disable Alpha Two (Late Compression)",
    )
    parser.add_argument(
        "--websocket",
        dest="websocket",
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Enable/disable WebSocket listener",
    )
    return parser.parse_args(argv)


def build_engine_config_from_cli_args(args: argparse.Namespace) -> EngineConfig:
    """Build engine config by merging environment defaults with CLI overrides.

    Args:
        args: Parsed CLI namespace produced by ``parse_cli_args``.

    Returns:
        Resolved ``EngineConfig`` with explicit CLI overrides applied.
    """
    config = EngineConfig.from_env()

    if args.mode is not None:
        config.mode = (
            TradingMode.LIVE if args.mode == MODE_LIVE else TradingMode.SIMULATION
        )
    if args.alpha_one is not None:
        config.enable_alpha_one = args.alpha_one
    if args.alpha_two is not None:
...
async def main(argv: Optional[List[str]] = None):
    """Run the unified trading engine from command-line inputs.

    Args:
        argv: Optional list of CLI tokens for parsing.
    """
